			...
			<div class="flex flex-col h-screen w-screen">
				<div class="w-full sm:w-96">
					...
					>
						...
						<Button
							v-if="!pendingRequest.data?.length && employeeDoc.doc && !isEditing"
							variant="subtle"
					...
					<div class="flex flex-col gap-4 mt-5 p-4">
						...
						<template v-else-if="employeeDoc.doc && employeeDocType.data">
							...
							<div class="flex flex-row items-center justify-between mt-2">
								...
								<Button v-if="isEditing" variant="outline" @click="addEducationRow">
									<template #prefix>
							...
							<div v-if="isEditing" class="flex flex-row gap-3 mt-3">
								<Button variant="subtle" class="w-full" @click="cancelEdit">
									{{ __("Cancel") }}
								</Button>
								<Button
									variant="solid"
...
// The document resource is cached across visits, so its onSuccess only fires for the
// first instance; watch the doc instead so the form is prefilled on every visit.
...
watch(
	() => employeeDoc.doc,
	(doc) => {
		if (!isEditing.value) fillForm(doc)
	},
	{ immediate: true }
)
